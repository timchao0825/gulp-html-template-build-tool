...
function iconFont() {
  return src(['src/images/font_svg/*.svg'], { base: './src/' })
    .pipe(
      changed('src/images/font_svg/*.svg', {
        extension: '.svg',
...
function sassDisplayError(error) {
  ...
  src('dist/*.html').pipe(inject.after('</head>', error_msg)).pipe(dest('dist'));
}
...
function jsFile() {
  return (
    src(['src/js/*.js', '!src/js/**/_*.js', '!src/js/{vendor,lib,plugin,plugins,foundation}/**/*.*'])
      .pipe(plumber())
      .pipe(babel())
      .pipe(jshint())
      .pipe(
...
function formatHTML() {
  return src(['dist/*.html']).pipe(formatHtml({ verbose: true }));
}
...
// watch file
function watchFiles() {
  watch('src/sass/**/*.+(scss|sass)', series(sassDelCommend, sassExportVendor, sassCompile, browsersyncReload));
  watch(
    ['src/js/**/*.js', '!src/js/**/_*.js', '!src/js/{vendor,lib,plugin,plugins,foundation}/**/*.*'],
